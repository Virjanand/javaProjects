...
public class Adverteerder_een_should extends AdoptiqTestScenario {
	...
	public void login() {
		 LoginFactory.loginAsAdverteerderEen(driver);
	...
	public void pay_buy_now_price() {
		...
		PurchaseConfirmation purchaseConfirmation = exchangePage.purchaseMedium(MEDIUM_BUY_NOW)
			.selectSeller(TestUser.UITGEVER_TWEE);
		...
		softly.assertThat(purchaseConfirmation.getFinalPrice())
			.as("Purchase Confirmation Final Price").isEqualTo("11.000,00");		
		...
		softly.assertThat(orders.getPrice(MEDIUM_BUY_NOW))
			.as("Order Page Paid Price").isEqualTo("11.000,00");	
		softly.assertAll();
	}
	
	public void pay_contract_price() {
		SoftAssertions softly = new SoftAssertions();
		...
		softly.assertThat(exchangePage.getFinalPrice(MEDIUM_RATE_CARD_CONTRACT))
			.as("Exchange Page Final Price").isEqualTo("2.062,50");
		...
		softly.assertThat(purchaseConfirmation.getTariefPrice())
			.as("Purchase Confirmation Tarief Price").isEqualTo("10.000,00");		
		softly.assertThat(purchaseConfirmation.getFinalPrice())
			.as("Purchase Confirmation Final Price").isEqualTo("5.500,00");		
		...
		softly.assertThat(orders.getPrice(MEDIUM_RATE_CARD_CONTRACT))
			.as("Order Page Paid Price").isEqualTo("5.500,00");	
		softly.assertAll();
	}
	...
	public void pay_rate_card_contract_price_with_volume_discount() {
		...
		softly.assertThat(purchaseConfirmation.getFinalPrice())
			.as("Purchase Confirmation Final Price").isEqualTo("2.062,50");		
		...
		softly.assertThat(orders.getPrice(MEDIUM_RATE_CARD_CONTRACT))
			.as("Order Page Paid Price").isEqualTo("2.062,50");	
		softly.assertAll();
	}
